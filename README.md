...
| 포스터 이미지 | TMDB API (python-dotenv로 키 관리) |
...
- Python 3.13 이상
- pip 패키지: `flask`, `flask-cors`, `PyJWT`, `bcrypt`, `python-dotenv`
...
```
TMDB_API_KEY=여기에_발급받은_키_입력
```

> TMDB API 키는 [themoviedb.org](https://www.themoviedb.org/) 회원가입 후 설정 → API에서 무료 발급
...
서버 시작 시 `.env`의 `TMDB_API_KEY`를 읽어 TMDB API로 영화 포스터와 배경 이미지를 자동으로 가져와 DB에 저장합니다.

- 포스터: `https://image.tmdb.org/t/p/w500` (영화 카드, 예매 페이지)
- 배경 이미지: `https://image.tmdb.org/t/p/w1280` (영화 상세 히어로 배경)

API 키가 없으면 장르별 CSS 그라디언트로 대체 표시됩니다.
